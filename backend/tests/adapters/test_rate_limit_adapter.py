...
async def test_first_increment_creates_row_with_count_one(
    ...
    count = await rate_limit.increment(USER_ID, window_start, expires_at)
    assert count == 1
...
async def test_subsequent_increment_increments_existing_row(
    ...
    window_start, expires_at = _window()
    await rate_limit.increment(USER_ID, window_start, expires_at)
    count = await rate_limit.increment(USER_ID, window_start, expires_at)
    assert count == 2
...
async def test_concurrent_increments_are_atomic(
    ...
    results = await asyncio.gather(
        *[rate_limit.increment(USER_ID, window_start, expires_at) for _ in range(10)]
    )
