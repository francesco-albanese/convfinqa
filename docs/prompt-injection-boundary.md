...
Suspicious-attempt throttling only inspects turns that a guardrail already blocked; allowed prompts are never throttled by this slice, so a patient attacker staying under the block threshold is not slowed down. The counter is per user and scope, so unauthenticated deployments (Cognito disabled) throttle on whatever identity the trust boundary provides. Throttling state lives in the shared `rate_limit` table, but `scope` is part of the primary key so a future general request limiter can reuse the table without colliding with suspicious-attempt counters. A full abuse platform (cross-session reputation, IP heuristics, automated lockout) remains out of scope.
